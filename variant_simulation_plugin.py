...
class VariantSimulationPlugin:
    ...
    def _load_module(self):
        ...
        module_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'simulate_variant_effect.py')
    ...
    def execute(self, method_name: str, params: dict, symbol_table: dict):
        """
        Ejecuta métodos relacionados con la simulación de variantes.
        """
        ...
        if method_name == "translate_dna":
            ...
            if dna_sequence is None or position is None or new_base is None:
                raise ValueError("Parameters 'dna_sequence', 'position', and 'new_base' are required for introduce_point_mutation.")
            
            mutated_sequence = self.simulate_variant_effect_module.introduce_point_mutation(dna_sequence, position, new_base)
