...
def test_bio_skills():
    ...
    payload = {
        ...
        "inputs": {"patient_id": "PAT-NEURO-001", "diplotypes": {"CYP2D6": "*4/*4"}, "drugs": ["fluoxetina"]},
    }
    ...
    payload = {
        ...
        "inputs": {"patient_id": "PAT-GER-001", "genotypes": {"APOE": "E4/E4"}},
    }
    r = requests.post(f"{base_url}/api/v2/skills/execute", json=payload).json()
    ...
    payload = {
        ...
        "inputs": {"patient_id": "PAT-NUT-001", "genotypes": {"MTHFR": "TT", "FADS1": "TT"}},
    }
    r = requests.post(f"{base_url}/api/v2/skills/execute", json=payload).json()
    print(f"Summary: {r['data']['summary']}")
